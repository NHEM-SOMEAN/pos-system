...
import {
  MdReceipt, MdRefresh, MdFileDownload, MdVisibility,
  MdArchive, MdUnarchive, MdAttachMoney, MdCheckCircle,
  MdHourglassEmpty, MdClose, MdFilterList
} from 'react-icons/md'
...
export default function Orders() {
  ...
  const fetchOrders = () => {
    setLoading(true)
    api.get('/orders', { params: { show_archived: showArchived } })
      .then(res => { setOrders(res.data.data || res.data); setLoading(false) })
  }
  ...
  const exportCSV = () => {
    const headers = ['Order ID', 'Cashier', 'Items', 'Total', 'Payment', 'Status', 'Date']
    const rows = filtered.map(o => [`#${o.id}`, o.user?.name || '-', o.items?.length || 0, `$${parseFloat(o.total).toFixed(2)}`, o.payment_method, o.status, new Date(o.created_at).toLocaleDateString()])
  ...
  const exportExcel = () => {
    const data = filtered.map(o => ({ 'Order ID': `#${o.id}`, 'Cashier': o.user?.name || '-', 'Total': parseFloat(o.total), 'Payment': o.payment_method, 'Status': o.status, 'Date': new Date(o.created_at).toLocaleDateString() }))
  ...
  const StatusBadge = ({ status }) => (
    <span style={{ background: status === 'paid' ? '#f0fdf4' : '#fffbeb', color: status === 'paid' ? '#16a34a' : '#d97706', padding: '4px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: '600', display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
      {status === 'paid' ? <MdCheckCircle size={12}/> : <MdHourglassEmpty size={12}/>}
      {status === 'paid' ? 'Paid' : 'Pending'}
    </span>
  )

  const PaymentBadge = ({ method }) => (
    <span style={{ background: method === 'cash' ? '#f0fdf4' : '#eff6ff', color: method === 'cash' ? '#16a34a' : '#2563eb', padding: '4px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: '600', display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
    ...
    <Layout>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <h1 style={{ fontSize: '20px', fontWeight: '700', color: '#0f172a', margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
            <MdReceipt size={22} style={{ color: '#3b82f6' }}/> Orders
          </h1>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          <button onClick={() => setShowArchived(!showArchived)} style={S.btn(showArchived ? '#fef3c7' : '#f1f5f9', showArchived ? '#d97706' : '#475569')}>
            {showArchived ? <MdReceipt size={14}/> : <MdArchive size={14}/>}
            {showArchived ? 'Active' : 'Archived'}
          </button>
          <div style={{ position: 'relative' }}>
            <button onClick={() => setShowExport(!showExport)} style={S.btn('#22c55e', 'white')}>
          ...
          <button onClick={fetchOrders} style={S.btn('#f1f5f9', '#475569')}><MdRefresh size={16}/></button>
        </div>
      </div>
      ...
      {!showArchived && (
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(3, 1fr)', gap: '12px', marginBottom: '20px' }}>
          {[
            { label: 'Revenue សរុប', value: `$${totalRevenue.toFixed(2)}`, icon: <MdAttachMoney size={22}/>, color: '#22c55e' },
            { label: 'Orders បានបង់', value: totalPaid, icon: <MdCheckCircle size={22}/>, color: '#3b82f6' },
            { label: 'Orders រង់ចាំ', value: totalPending, icon: <MdHourglassEmpty size={22}/>, color: '#f59e0b' },
          ].map(card => (
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '16px', flexWrap: 'wrap', alignItems: 'center' }}>
        <div style={{ position: 'relative', flex: isMobile ? '1 1 100%' : '0 0 200px' }}>
          <MdFilterList size={16} style={{ position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)', color: '#94a3b8' }}/>
          <input placeholder="Order ID / Cashier..." value={search} onChange={e => setSearch(e.target.value)}
        ...
        {(dateFrom || dateTo) && (
          <button onClick={() => { setDateFrom(''); setDateTo('') }} style={{ background: '#fee2e2', color: '#dc2626', border: 'none', padding: '8px', borderRadius: '8px', cursor: 'pointer', display: 'flex' }}>
            <MdClose size={16}/>
          </button>
        )}
      </div>
      ...
      {isMobile ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: '40px', color: '#94a3b8' }}><MdReceipt size={40} style={{ opacity: 0.3 }}/><div>គ្មាន order</div></div>
          ) : filtered.map(o => (
            <div key={o.id} style={{ background: 'white', borderRadius: '12px', padding: '14px', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
              ...
              <div style={{ display: 'flex', gap: '6px' }}>
                <button onClick={() => setSelected(o)} style={{ flex: 1, background: '#eff6ff', color: '#2563eb', border: 'none', padding: '7px', borderRadius: '8px', fontSize: '12px', fontWeight: '600', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px' }}>
                  <MdVisibility size={14}/> Detail
                </button>
                {showArchived
                  ? <button onClick={() => handleUnarchive(o.id)} disabled={archiving === o.id} style={{ flex: 1, background: '#f0fdf4', color: '#16a34a', border: 'none', padding: '7px', borderRadius: '8px', fontSize: '12px', fontWeight: '600', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px' }}>
                      <MdUnarchive size={14}/> Unarchive
                    </button>
                  : <button onClick={() => handleArchive(o.id)} disabled={archiving === o.id} style={{ flex: 1, background: '#fffbeb', color: '#d97706', border: 'none', padding: '7px', borderRadius: '8px', fontSize: '12px', fontWeight: '600', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan="8" style={{ textAlign: 'center', padding: '48px', color: '#94a3b8' }}>គ្មាន order</td></tr>
              ) : filtered.map((o, i) => (
                <tr key={o.id} style={{ borderBottom: i < filtered.length - 1 ? '1px solid #f1f5f9' : 'none' }}
                  ...
                  <td style={{ padding: '12px 16px' }}><PaymentBadge method={o.payment_method}/></td>
                  <td style={{ padding: '12px 16px' }}><StatusBadge status={o.status}/></td>
                  <td style={{ padding: '12px 16px', fontSize: '12px', color: '#94a3b8' }}>{new Date(o.created_at).toLocaleDateString('km-KH')}</td>
                  <td style={{ padding: '12px 16px' }}>
                    <div style={{ display: 'flex', gap: '6px' }}>
                      <button onClick={() => setSelected(o)} style={{ background: '#eff6ff', color: '#2563eb', border: 'none', padding: '5px 10px', borderRadius: '6px', fontSize: '11px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <MdVisibility size={12}/> Detail
                      </button>
                      {showArchived
                        ? <button onClick={() => handleUnarchive(o.id)} disabled={archiving === o.id} style={{ background: '#f0fdf4', color: '#16a34a', border: 'none', padding: '5px 10px', borderRadius: '6px', fontSize: '11px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px' }}>
                            <MdUnarchive size={12}/> Unarchive
                          </button>
                        : <button onClick={() => handleArchive(o.id)} disabled={archiving === o.id} style={{ background: '#fffbeb', color: '#d97706', border: 'none', padding: '5px 10px', borderRadius: '6px', fontSize: '11px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px' }}>
                            <MdArchive size={12}/> Archive
                          </button>
                      }
                    </div>
                  </td>
          ...
          <div style={{ background: 'white', borderRadius: '16px', padding: '24px', width: '100%', maxWidth: '500px', maxHeight: '85vh', overflow: 'auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
              <h2 style={{ fontSize: '16px', fontWeight: '700', color: '#0f172a', margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
                <MdReceipt size={18} style={{ color: '#3b82f6' }}/> Order #{selected.id}
              </h2>
              <button onClick={() => setSelected(null)} style={{ background: '#f1f5f9', border: 'none', borderRadius: '8px', padding: '6px', cursor: 'pointer', display: 'flex' }}>
