...
import {
  MdShoppingCart, MdSearch, MdRemove, MdAdd, MdDelete,
  MdAttachMoney, MdQrCode, MdClose, MdInventory, MdWarning,
  MdCheckCircle
} from 'react-icons/md'
...
export default function POS() {
  ...
  const handleCheckout = async (method) => {
    ...
    if (method === 'khqr') {
      setQrModal(true); setQrPolling(true); setLoading(true)
      try {
        const orderRes = await api.post('/orders', {
          items: cart.map(i => ({ product_id: i.id, quantity: i.quantity })),
          payment_method: 'khqr', discount,
        })
        const khqrRes = await api.post('/khqr/generate', { amount: total, order_id: orderRes.data.id })
        setQrOrder({ ...orderRes.data, qr: khqrRes.data.qr, md5: khqrRes.data.md5 })
        const pollInterval = setInterval(async () => {
          try {
            const checkRes = await api.post('/khqr/check', { md5: khqrRes.data.md5, order_id: orderRes.data.id })
            if (checkRes.data.status === 'paid') {
              clearInterval(pollInterval); setQrPolling(false)
              const orderDetail = await api.get(`/orders/${orderRes.data.id}`)
              setReceiptOrder(orderDetail.data); clearCart()
              setQrModal(false); setQrOrder(null)
    ...
    try {
      const res = await api.post('/orders', {
        items: cart.map(i => ({ product_id: i.id, quantity: i.quantity })),
        payment_method: method, discount,
      })
  ...
  const S = {
    searchInput: { width: '100%', border: '1px solid #e2e8f0', borderRadius: '10px', padding: '10px 16px 10px 40px', fontSize: '13px', outline: 'none', boxSizing: 'border-box' },
    ...
    productCard: (outOfStock) => ({ background: 'white', border: '1px solid #e2e8f0', borderRadius: '12px', padding: '14px', cursor: outOfStock ? 'not-allowed' : 'pointer', opacity: outOfStock ? 0.5 : 1, transition: 'box-shadow 0.15s' }),
    ...
    btnClear: { width: '100%', background: 'transparent', color: '#ef4444', border: 'none', fontSize: '12px', cursor: 'pointer', padding: '6px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' },
  }
    ...
    <>
      <div style={S.cartHeader}>
        <span style={{ fontSize: '14px', fontWeight: '700', color: '#0f172a', display: 'flex', alignItems: 'center', gap: '8px' }}>
          <MdShoppingCart size={18} style={{ color: '#3b82f6' }}/> {t('cart')}
        </span>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          {isMobile && (
            <button onClick={() => setShowCart(false)} style={{ background: '#f1f5f9', border: 'none', borderRadius: '6px', padding: '4px', cursor: 'pointer', display: 'flex' }}>
              <MdClose size={16} style={{ color: '#64748b' }}/>
            </button>
          )}
        </div>
      </div>
      ...
      <div style={S.cartItems}>
        {cart.length === 0 ? (
          <div style={{ textAlign: 'center', marginTop: '40px', color: '#94a3b8' }}>
            <MdShoppingCart size={48} style={{ opacity: 0.3, marginBottom: '8px' }}/>
          ...
          <div key={item.id} style={S.itemRow}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              <button style={S.qtyBtn('#e2e8f0')} onClick={() => updateQty(item.id, item.quantity - 1)}>
                <MdRemove size={14}/>
              </button>
              <span style={{ fontSize: '13px', fontWeight: '700', minWidth: '20px', textAlign: 'center' }}>{item.quantity}</span>
              <button style={S.qtyBtn('#3b82f6')} onClick={() => updateQty(item.id, item.quantity + 1)}>
                <MdAdd size={14}/>
              </button>
            </div>
            <button onClick={() => removeFromCart(item.id)} style={{ background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer', display: 'flex', padding: '2px' }}>
              <MdClose size={16}/>
            </button>
          </div>
        ))}
      ...
      <div style={S.cartFooter}>
        ...
        <button style={{ ...S.btnGreen, opacity: loading || cart.length === 0 ? 0.6 : 1 }} onClick={() => handleCheckout('cash')} disabled={loading || cart.length === 0}>
          {loading ? <Spinner size={16} color="white"/> : <MdAttachMoney size={18}/>} {t('cash')}
        </button>
        <button style={{ ...S.btnBlue, opacity: loading || cart.length === 0 ? 0.6 : 1 }} onClick={() => handleCheckout('khqr')} disabled={loading || cart.length === 0}>
          {loading ? <Spinner size={16} color="white"/> : <MdQrCode size={18}/>} {t('khqr')}
        </button>
        {cart.length > 0 && (
          <button style={S.btnClear} onClick={clearCart}>
            <MdDelete size={14}/> {t('clearCart')}
          </button>
        )}
      </div>
    </>
  ...
  return (
    <Layout>
      ...
      <div style={{ display: 'flex', gap: '20px', height: isMobile ? 'auto' : 'calc(100vh - 140px)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(auto-fill, minmax(160px, 1fr))', gap: '12px', overflowY: isMobile ? 'visible' : 'auto', paddingBottom: isMobile ? '80px' : '8px' }}>
            {filtered.length === 0 ? (
              <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: '40px', color: '#94a3b8' }}>
                <MdInventory size={48} style={{ opacity: 0.3, marginBottom: '8px' }}/>
                <div>{t('noProducts')}</div>
              </div>
            ) : filtered.map(product => (
              <div key={product.id} style={S.productCard(product.stock <= 0)} onClick={() => addToCart(product)}>
                <div style={{ background: '#f8fafc', borderRadius: '8px', height: '100px', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '8px', overflow: 'hidden' }}>
                  {product.image
                    ...
                    : <MdInventory size={32} style={{ color: '#cbd5e1' }}/>
                  }
                </div>
                ...
                <div style={{ fontSize: '11px', color: product.stock <= 5 ? '#ef4444' : '#94a3b8', display: 'flex', alignItems: 'center', gap: '3px' }}>
                  {product.stock <= 5 && <MdWarning size={12}/>}
        ...
        {/* RIGHT - Cart Desktop */}
        {!isMobile && (
          <div style={{ width: '320px', background: 'white', border: '1px solid #e2e8f0', borderRadius: '14px', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
            <CartContent/>
          </div>
        )}
      </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '20px' }}>
          <div style={{ background: 'white', borderRadius: '16px', padding: '24px', width: '100%', maxWidth: '340px', textAlign: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', marginBottom: '4px' }}>
              <MdQrCode size={24} style={{ color: '#0066cc' }}/>
            ...
            <div style={{ background: 'white', border: '3px solid #0066cc', borderRadius: '12px', padding: '16px', marginBottom: '16px' }}>
              ...
              <div style={{ marginTop: '8px', fontSize: '10px', color: '#64748b' }}>National Bank of Cambodia</div>
            </div>
            ...
            {qrPolling ? (
              <div style={{ color: '#64748b', fontSize: '13px', marginBottom: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
              ...
              <div style={{ color: '#16a34a', fontWeight: '600', marginBottom: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
                <MdCheckCircle size={18}/> Payment Confirmed!
              </div>
            )}
            <button onClick={() => { setQrModal(false); setQrOrder(null); setQrPolling(false) }}
              style={{ background: '#fee2e2', color: '#ef4444', border: 'none', padding: '10px 24px', borderRadius: '8px', cursor: 'pointer', fontWeight: '600', display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
              <MdClose size={16}/> បោះបង់
            </button>
          </div>
        </div>
      )}
